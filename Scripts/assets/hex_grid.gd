...
@export_range(10, 80) var grid_size := 20
var radius := grid_size / 2

func _ready() -> void:
	randomize()
	_generate_grid()

func is_water(x: int, y: int, center: Vector2, dist: float, max_dist: float) -> bool:
	# --- OCEAN RING (outer 10%) ---
	...
	if dist > 5:
		# blob noise using randf seeded by coordinates
		var n := hash(x * 928371 + y * 192837) % 1000 / 1000.0
		if n < 0.15:  # probability of lake tile
			return true
...
func _generate_grid():
	...
	for x in range(grid_size):
		for y in range(grid_size):
			...
			if dist > max_dist:
				continue  # outside circle → skip tile entirely
			...
			tile_coordinates.z = (
				y * TILE_SIZE * SPACING +
				(0.0 if x % 2 == 0 else (TILE_SIZE * SPACING) / 2)
			)
